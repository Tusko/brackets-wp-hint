...
const funcParser = htmlToJson.createParser(['.section#functions table tbody tr', {
  'func': $el => $el.find('.td-name a[href]').text().trim(),
  'method': $el => $el.next().find('.td-description').text()
}]);

console.log("Started to load ACF functions");
const timerStart = Date.now();

funcParser.request('https://www.advancedcustomfields.com/resources/').then(links => {
  for(f in links) {
